...
- [claude-debug](https://github.com/krabat-l/claude-debug) - Phase-gated debugging — hooks block code edits until root cause is reproduced, isolated, and confirmed with the user. 6 bug-type strategies (crash, wrong-output, intermittent, regression, performance, test-failure), 3 read-only agents, zero-install option.
